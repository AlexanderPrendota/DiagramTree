...
<p>
When browser elements are focused, they are scrolled into view as much as possible, which may be unwelcome in some web apps. You can disable this effect by overriding <code>Diagram.doFocus</code>.

<pre data-language="javascript">// Save and restore the window scroll values
...
<p>
HTMLInfos used as text editors must also define a <a>HTMLInfo.valueFunction</a>. On <a>TextEditingTool.acceptText</a> is called, GoJS will call <code>HTMLInfo.valueFunction</code> and use the return value as the value for the TextEditingTool completion.
